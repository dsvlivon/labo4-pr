...
import { collectionData, Firestore, addDoc, collection, deleteDoc, doc, updateDoc, DocumentReference, DocumentData } from '@angular/fire/firestore';
...
export class FirebaseService {
  ...
  setData2(obj: any, coleccion: string): Promise<DocumentReference<any, DocumentData>> {
    const col = collection(this.fireStore, coleccion);
    return addDoc(col, obj);
  }
  ...
  actualizarHelado(obj: any) {
    const dataRef = doc(this.fireStore, 'helados', obj.id);
    const updateData: any = {};
    updateData.tipo = obj.tipo;
    updateData.precio = obj.precio;
    updateData.peso = obj.peso;
